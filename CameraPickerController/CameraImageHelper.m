...
//-(void) updatePreviewInView:(UIView*) aView {
//    if(!session) return;
//    self.preview.frame = aView.bounds;
//    self.preview.videoGravity = AVLayerVideoGravityResizeAspectFill;
//
...
-(void) takePicture
{
    ...
    [captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         CFDictionaryRef attachments = CMGetAttachment(imageSampleBuffer, kCGImagePropertyExifDictionary, NULL);
//         if (exifAttachments) {
...
//         }
         ...
         [self.delegate didFinishedTakePicture:image attachments:attachments];
     }];
}
